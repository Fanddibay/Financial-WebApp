...
<template>
  <BottomSheet
    :is-open="isOpen"
    :title="t('pocket.limitUpgrade.title')"
    @close="emit('close')"
  >
    <div class="flex flex-col items-center gap-4 py-2">
      <div
        class="relative flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-brand/10 text-brand dark:bg-brand/20"
      >
        <font-awesome-icon :icon="['fas', 'layer-group']" class="h-8 w-8" />
        <span
      ...
      <p class="text-center text-sm text-slate-600 dark:text-slate-400">
        {{ t('pocket.limitUpgrade.description') }}
      </p>
    </div>
    ...
    <template #footer>
      <div class="flex flex-col gap-2">
        <BaseButton class="w-full" size="lg" @click="handleUpgrade">
          {{ t('pocket.limitUpgrade.upgrade') }}
        </BaseButton>
        <BaseButton variant="secondary" class="w-full" @click="handleCancel">
          {{ t('pocket.limitUpgrade.cancel') }}
        </BaseButton>
      </div>
    </template>
  </BottomSheet>
