...
<template>
  <BottomSheet
    :is-open="isOpen"
    :title="t('goal.limitUpgrade.title')"
    @close="emit('close')"
  >
    <div class="flex flex-col items-center gap-4 py-2">
      ...
      >
        <font-awesome-icon :icon="['fas', 'bullseye']" class="h-8 w-8" />
        <span
      ...
      <p class="text-center text-sm text-slate-600 dark:text-slate-400">
        {{ t('goal.limitUpgrade.description') }}
      </p>
    </div>
