...
async function handleSend() {
  ...
  if (!tokenStore.canUseChat()) {
    limitError.value = t('chat.limitReachedMessage')
    setTimeout(() => {
  ...
  <div v-if="chatStore.isOpen"
    ...
    <div
      ...
      <div class="flex items-center gap-2">
        ...
        <div>
          <h3 class="font-semibold text-white">{{ t('chat.headerTitle') }}</h3>
          <p class="text-xs text-white/80">{{ t('chat.headerSubtitle') }}</p>
        </div>
      </div>
      <div class="flex items-center gap-2">
        <!-- Delete Icon -->
    ...
    <div class="flex-1 space-y-4 overflow-y-auto p-4 bg-slate-50 dark:bg-slate-800">
      <div v-if="!hasMessages" class="flex h-full flex-col items-center justify-center text-center px-4 py-8">
        <div class="space-y-4 w-full max-w-xs">
          ...
          <div>
            <p class="text-slate-800 dark:text-slate-200 font-semibold text-lg">{{ t('chat.emptyTitle') }}</p>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ t('chat.emptySubtitle') }}</p>
          </div>
          ...
          <div class="pt-4 border-t border-slate-200 dark:border-slate-700">
            <button @click="showInsights = !showInsights"
              ...
              <span>{{ t('chat.insightsButtonLabel') }}</span>
