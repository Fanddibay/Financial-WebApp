...
const miniInsights = computed(() => {
  ...
  // 4. Pocket/Goal usage (if room and multiple destinations used)
  if (insights.length < 2 && tx.length >= 2) {
    const byDest = new Map<string, number>()
    for (const t of tx) {
      const name = t.goalId
        ? (goalStore.getGoalById(t.goalId)?.name ?? t.goalId)
        : (pocketStore.getPocketById(t.pocketId)?.name ?? t.pocketId)
      byDest.set(name, (byDest.get(name) ?? 0) + 1)
    }
    const dests = [...byDest.entries()]
    if (dests.length >= 2) {
      const top = dests.sort((a, b) => b[1] - a[1])[0]
      if (top && top[1] >= 2) {
        insights.push(t('notificationCenter.insightPocket', { pocket: top[0] }))
      }
    }
  }
...
function destinationLabel(tx: Transaction): string {
  ...
  return pocketStore.getPocketById(tx.pocketId)?.name ?? '-'
}
...
<template>
  <BottomSheet :is-open="isOpen" :title="sheetTitle" :subtitle="sheetSubtitle" max-height="65" @close="emit('close')">
    ...
    <template v-if="viewMode === 'list'">
      <div class="notification-list-root">
        ...
        <div v-if="!profileStore.profile.notificationsEnabled"
        ...
        <TransitionGroup v-else tag="div" name="list" class="space-y-2">
          <div v-for="item in notificationItems" :key="item.id" role="button" tabindex="0" :class="[
          ...
          ]" @click="handleItemClick(item)" @keydown.enter.space.prevent="handleItemClick(item)">
            ...
            ]" :aria-label="isRead(item.id) ? 'Open' : 'Unread'" @click.stop="handleEnvelopeClick($event, item)">
              <font-awesome-icon :icon="['fas', isRead(item.id) ? 'envelope-open' : 'envelope']" class="h-5 w-5" />
            </button>
            ...
            <button type="button"
              ...
              :aria-label="t('common.delete')" @click.stop="handleDelete($event, item)">
      ...
      <div class="space-y-4">
        ...
        <template v-else>
          ...
          <div class="rounded-xl bg-amber-50/80 px-4 py-3 dark:bg-amber-900/20">
            <div class="flex gap-3">
              ...
              <div class="min-w-0 flex-1 space-y-1">
                ...
                <template v-else>
                  <p v-for="(insight, i) in miniInsights" :key="i"
          ...
          <div>
            ...
            <div class="space-y-2">
              <button v-for="tx in detailTransactions" :key="tx.id" type="button"
  ...
  <Teleport to="body">
    <Transition enter-active-class="transition-opacity duration-200" enter-from-class="opacity-0"
      ...
      <div v-if="selectedTx" class="fixed inset-0 z-[60] flex flex-col justify-end bg-black/50"
        ...
        <Transition enter-active-class="transition-transform duration-300 ease-out" enter-from-class="translate-y-full"
          ...
          <div v-if="selectedTx" class="rounded-t-2xl bg-white p-4 pb-8 dark:bg-slate-800" @click.stop>
